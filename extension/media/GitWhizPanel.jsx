...
import RetroPulse from "./RetroPulse";
...
const GitWhizApp = () => {
  ...
  const [activeTab, setActiveTab] = useState('gitwhiz'); // or 'retropulse'
  ...
  return (
    <div className="fixed inset-0 font-mono bg-black text-green-500 flex flex-col text-[0.95em]">
        <div className="flex flex-row items-end text-right space-y-1 gap-20" >
      ...
      <div className="flex flex-col flex-1 p-2 overflow-hidden">
        {/* Tabs + User Info + Logout */}
        <div className="flex items-center justify-between mb-2 px-1">
          <div className="flex space-x-4">
            <button
              onClick={() => setActiveTab('gitwhiz')}
              className={`px-3 py-1 border-b-2 ${activeTab === 'gitwhiz' ? 'border-green-400 text-green-400' : 'border-transparent text-green-700'
                } hover:text-green-300 transition text-sm`}
            >
              GitWhiz Terminal
            </button>
            <button
              onClick={() => setActiveTab('retropulse')}
              className={`px-3 py-1 border-b-2 ${activeTab === 'retropulse' ? 'border-green-400 text-green-400' : 'border-transparent text-green-700'
                } hover:text-green-300 transition text-sm`}
            >
              RetroPulse
            </button>
